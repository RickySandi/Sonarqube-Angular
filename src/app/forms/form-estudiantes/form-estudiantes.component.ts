...
export class FormEstudiantesComponent implements OnInit {
  ...
  async init(id: any) {
    ...
    if (this.esNuevo) {
      ...
      var estudiantesDeGestionIngresada = this.estudiantes.filter(
        (estudiante: Estudiante) =>
          estudiante.gestionRegistro === this.estudiante.gestionRegistro
      );
      if (estudiantesDeGestionIngresada.length === 0) {
        this.estudiante.numGestion = "1";
      } else {
        this.estudiante.numGestion = (
          Math.max.apply(
            Math,
            estudiantesDeGestionIngresada.map(function (
              estudiante: Estudiante
            ) {
              return Number(estudiante.numGestion);
            })
          ) + 1
        ).toString();
      }
      if (this.estudiantes.length == 0) {
        ...
        this.estudiante.numHistorico = (
          Math.max.apply(
            Math,
            this.estudiantes.map(function (estudiante: Estudiante) {
              return Number(estudiante.numHistorico);
            })
          ) + 1
        ).toString();
      }
  ...
  validarEstudiante(mostrarError = false): boolean {
    ...
    this.validacion.nombres =
      ...
      /^[A-Z][A-zÀ-ú ]+$/.test(this.estudiante.nombres);
    this.validacion.apellidos =
      this.estudiante.apellidos != "" &&
      /^[A-Z][A-zÀ-ú ]+$/.test(this.estudiante.apellidos);
    this.validacion.fecNacimiento =
      this.estudiante.fecNacimiento < moment().format("YYYY-MM-DD") + 1 &&
      this.estudiante.fecNacimiento != "";
    this.validacion.numHistorico =
      this.estudiante.numHistorico != "" &&
      /^[0-9]+$/.test(this.estudiante.numHistorico);
    this.validacion.numGestion =
      this.estudiante.numGestion != "" &&
      /^[0-9]+$/.test(this.estudiante.numGestion);
    this.validarGestionRegistro();
    this.validacion.direccion =
    ...
    return (
      ...
      this.validacion.numHistorico &&
      this.validacion.numGestion &&
      this.validacion.gestionRegistro &&
      this.validacion.direccion &&
      this.validacion.fecNacimiento &&
  ...
  validarRegistro() {
    if (
      this.estudiante.estados.length != 0 &&
      this.estudiante.diagnosticos.length != 0
    ) {
          ...
          this.validacion.fechaRegistro = this.estudiante.estados.every(
            (estado: any) =>
              moment(estado.fecha).format("YYYY") >=
                this.estudiante.gestionRegistro &&
              moment(estado.fecha).format("YYYY-MM-DD") >=
                this.estudiante.fecNacimiento
          );
  ...
  verificarSiRegistroVacio(index: any) {
    return (
      this.estudiante.estados[index].estado == "" &&
      this.estudiante.estados[index].fecha == "" &&
      this.estudiante.diagnosticos[index].diagnostico == ""
    );
  }
  validarGestionRegistro() {
    this.validacion.gestionRegistro =
      this.estudiante.gestionRegistro != "" &&
      /^[0-9]+$/.test(this.estudiante.gestionRegistro);
    if (this.estudiante.fecNacimiento != "") {
      this.validacion.gestionRegistro =
        this.estudiante.gestionRegistro >=
        moment(this.estudiante.fecNacimiento).format("YYYY");
  ...
  async guardarEstudiante(tipoCreacion: string = "agregarSinRedirigir") {
    if (this.validarEstudiante(true)) {
      ...
      if (moment(this.estudiante.fecNacimiento).format("M") >= "9") {
        this.estudiante.mes =
          "0" + moment(this.estudiante.fecNacimiento).format("M");
      } else {
      ...
      this.idEstudianteAuxiliar =
        await this.estudiantesService.guardarEstudiante(
          this.estudiante,
          this.esNuevo
        );
  ...
  irDetalleEstudiante() {
    ...
    console.log(id);
    if (id != "") this.router.navigateByUrl(`detalles-estudiante/${id}`);
    else this.router.navigateByUrl("/lista-estudiantes");
  }
  ...
  abrirModalEliminarRegistro(index: number) {
    ...
    modalRef.componentInstance.delete.subscribe((confirm: boolean) => {
      if (confirm) {
        this.listaSesionesPorBorrar.push(
          this.dicSesiones[this.estudiante.sesiones[index].id]
        );
  ...
  cargarNumeroGestion() {
    if (this.esNuevo) {
      ...
      if (
        this.estudiante.gestionRegistro !=
        this.estudianteGestionRegistroAuxiliar
      ) {
        var estudiantesDeGestionIngresada = this.estudiantes.filter(
          (estudiante: Estudiante) =>
            estudiante.gestionRegistro === this.estudiante.gestionRegistro
        ...
        this.estudianteNumHistoricoAuxiliar =
          this.estudianteNumHistoricoAuxiliar;
      }
    }
  }
  ...
  async dirigirApoderado(id: string) {
    if (this.validarEstudiante(true)) {
      ...
      this.router.navigate(["/form-apoderados"], {
        queryParams: { id, auxIdEstudiante },
      });
    }
  }
    ...
    if (this.validarRegistro()) {
      if (this.validarEstudiante(true)) {
        if (this.validarQuelosRegistrosestenLlenos()) {
          ...
          if (!this.esNuevo) {
            this.router.navigate(["/form-sesiones"], {
              queryParams: { id, estudiante, index },
            });
          } else {
            var seEstaregistrando = true;
            estudiante = this.idEstudianteAuxiliar;
            this.router.navigate(["/form-sesiones"], {
              queryParams: { id, estudiante, index, seEstaregistrando },
            });
          }
        } else {
          this.abrirModalError(
            ": No se puede agregar sesión si existe algún registro vacío"
          );
        }
      } else {
        this.abrirModalError();
      ...
      this.validacion.mostrarError = true;
      this.abrirModalError(
        ": No se puede agregar sesión si existe algún error en los registros"
      );
    }
  }
  ...
  initDicSesiones() {
    this.sesiones.forEach((sesion: Sesion) => {
      this.dicSesiones[sesion.id] = sesion;
    });
  }
