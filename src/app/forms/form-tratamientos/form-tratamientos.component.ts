...
export class FormTratamientosComponent implements OnInit {
  ...
  validarTratamiento(mostrarError = false): boolean {
    ...
    this.validacion.tratamiento =
      ...
      /^[A-Z][A-zÀ-ú ]+$/.test(this.tratamiento.tratamiento);
    this.validacion.descripcion =
      this.tratamiento.descripcion != "" &&
      /^[A-Z][A-zÀ-ú ]+$/.test(this.tratamiento.descripcion);
    return this.validacion.tratamiento && this.validacion.descripcion;
  }
  ...
  async guardarTratamiento() {
    if (this.validarTratamiento(true)) {
      ...
      await this.tratamientosService.guardarTratamiento(
        this.tratamiento,
        this.esNuevo
      );
      this._location.back();
