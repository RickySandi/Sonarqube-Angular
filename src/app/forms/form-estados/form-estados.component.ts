...
export class FormEstadosComponent implements OnInit {
  ...
  validarEstado(mostrarError = false): boolean {
    ...
    this.validacion.numEstado =
      this.estado.numEstado != "" && /^[0-9]+$/.test(this.estado.numEstado);

    if (this.esNuevo) {
      this.validacion.numEstado =
        this.estado.numEstado &&
        !this.estados.some(
          (estado: Estado) => estado.numEstado === this.estado.numEstado
        );
    }
    this.validacion.estado =
      this.estado.estado != "" && /^[A-Z][A-zÀ-ú ]+$/.test(this.estado.estado);
    return this.validacion.numEstado && this.validacion.estado;
  }
